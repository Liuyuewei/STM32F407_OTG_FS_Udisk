...
//#include "MagicBlueCmos.h"

extern uint8_t U_DISK[];
...
{
  uint32_t len;
	static uint32_t blk_address=0;
  
  len = MIN(SCSI_blk_len , MSC_MEDIA_PACKET); 
	
  blk_address=SCSI_blk_addr / SCSI_blk_size;
// 		if(blk_address>=0x1e)          //读到了 数据交换区域
// 	{
// 	  DataReadSign=1;
...
//   }
	blk_address<<=9;   //<<9 bit 相当于 blk_address*512   注释是  old 情况
